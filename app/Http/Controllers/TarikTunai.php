...
class TarikTunai extends Controller
{
    ...
    public function store(Request $request)
    {
		...
		if($cek_siswa!=null and $tabsiswa!=null and $cek_tabungan!=null){
			...
			if ($tabsiswa->saldo >= $request->nominal and $statusTabungan){
				...
				$kode_transaksi = $this->generate_kode_transaksi_tarik($request->nis,$request->jenis_tabungan);
	...
	public function generate_kode_transaksi_tarik($nis,$jenis){
        $count = DB::table('transaksi')->where('nis',$nis)->get()->count()+1;
        $category =  substr(strtoupper($jenis),0,3);
        return("T".$count.$category.$nis);
    }

	//Mengirim noif ke mobile
