...
class SetorTunaiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($cek_siswa!=null and $tabsiswa!=null and $cek_tabungan!=null ){
            ...
            $kode_transaksi = $this->generate_kode_transaksi_setor($request->nis,$request->jenis_tabungan);
    ...
    public function generate_kode_transaksi_setor($nis,$jenis){
        $count = DB::table('transaksi')->where('nis',$nis)->get()->count()+1;
        $category =  substr(strtoupper($jenis),0,3);
        return("S".$count.$category.$nis);
    }
    
    //Mengirim noifikasi ke mobile
